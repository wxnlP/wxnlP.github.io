...
- 【库文件链接】
  ...
  - `-l`：指定要链接的 **库文件名称**
    ...
    - > `-l` 参数紧接着就是库名，例如：数学库为<kbd>libm.so</kbd>，参数就是`-lm`，即去掉`lib`和`.so`。此外，放在 `/lib` 、`/usr/lib`和`/usr/local/lib` 里的库直接用 -l 参数就能链接，其他位置的库文件需要指定目录路径。

  - `-L`：指定要链接的 **库文件路径**

    - > 除 `/lib` 、`/usr/lib`和`/usr/local/lib`以外，其他目录的库，需要使用`-L`参数指定目录路径。

  - `-I`：指定 **头文件目录**

    - > `/usr/include`目录一般是不用指定，其他目录则需要指定，可以用相对路径。
