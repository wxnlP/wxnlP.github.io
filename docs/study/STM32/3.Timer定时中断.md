...
!!! warning
    警告：--基本定时器仅支持向上计数--时钟选择也仅支持内部时钟，为72MHZ
...
可以在`stm32f1xx_it.c`文件找到`TIM2_IRQHandler`函数
...
总览一下中断处理函数：`HAL_TIM_IRQHandler(&htim2)`
