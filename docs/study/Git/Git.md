...
**使用场景**：正在 `dev` 分支写代码（有未完成的修改和新文件），突然需要切到 `main` 修复紧急 Bug 或拉取最新代码。

| **步骤**    | **操作说明**                     | **Git 指令**        |
| ----------- | -------------------------------- | ------------------- |
| **1. 储藏** | 暂存当前所有修改（含未追踪文件） | `git stash -u`      |
| **2. 切换** | 切换到主分支处理事务             | `git checkout main` |
| **3. 回归** | 事情办完，切回开发分支           | `git checkout dev`  |
| **4. 同步** | （可选）获取 `main` 的最新代码   | `git rebase main`   |
| **5. 恢复** | 弹出之前存的代码，继续工作       | `git stash pop`     |

**进阶补充：**

- `git stash list`：查看所有储藏记录。
- `git stash apply`：恢复代码但不删除储藏记录（比 `pop` 更保险）。
- `git stash clear`：清空所有储藏。

### 压缩commits

**使用场景**：你在开发分支提交了大量琐碎的 commit（如 "fix typo"），但在合并进 `main` 前，希望将它们合并为一个完整的“功能块”。

| **步骤**        | **操作说明**                         | **Git 指令**                           |
| --------------- | ------------------------------------ | -------------------------------------- |
| **1. 准备**     | 切换到目标主分支                     | `git checkout main`                    |
| **2. 压缩合并** | 将 dev 变动带过来但不产生多个 commit | `git merge --squash dev`               |
| **3. 提交**     | 手动创建一个整洁的提交信息           | `git commit -m "feat: [核心功能描述]"` |

###  rebase

把远程的更新拉下来，并把本地修改“顶”到最前端。
